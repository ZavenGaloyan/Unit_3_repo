...
| ------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Registration System | Unit Test        | 1. Run the Python file. 2.Click registration button. 3. Fill in text fields accordingly. 4. Press register button                                           | Login screen opens then once the register is pressed the registrations page comes up. If text fields have invalid inputs then program provides helper text for the inputs to be valid. Once the inputs are valide teh registration will go through and the page will change back to the login screen.                                                                    |
| Login System        | Unit Test        | 1. Run the Python file. 2. Enter login credtionals in textfields. 3. Click Login button                                                                     | Login screen opens up . Once creditionals are entered into the text the fields. Click Login button . If login is incorrect message appears that Login is incorrect. Also if text fields are empty then message pops up to say taht text fields are empty. Once the login credintionals match that off the ones in the database the page changes to the home screen.      |
| View Info page      | Unit Test        | 1. Run the Python file. 2. Login in. 3. Click on View info button.                                                                                          | Once Login has been completed home screen will apear. When the View Info button has been press a table will apear with all of the data inputed by that individual user.                                                                                                                                                                                                 |
| Adding info         | Integration Test | 1. Run the Python file. 2. Login in. 3. Click on View info button. 4. Click on the Add button. 5. Fill in text fields . 6. Click Enter into database button | When the add button is clicked a new page will aprear to add information to the database. It will allow the user to choose an exercise and enter inputs. If the inputs are empty or invalid then a helper text will apear. Once the information is valid the data will be entered into the databse and page changed to viewinfo page. The entry will apear in the table. |
| Delete function     | Integration Test | 1. Run the Python file. 2. Login in. 3. Click on View info button. 4. click box in given row in table.                                                      | When a box is clicked it on the table it becomes blue. When the detele button is pressed it removes that entry from the database. When the View info page is left then returend to the deleted entry will be gone.                                                                                                                                                       |
| Calendar Function   | Unit Test        | 1. Run the Python file. 2. Login in. 3. Click on the Calendar button                                                                                        | When the caledar button is pressed the caleder for the current month will apear. Based off the database date and exercise it will color code certain days for the specifc exercise that was trained.                                                                                                                                                                     |
| Explore page        | Unit Test        | 1. Run the Python file. 2. Login in. 3. Click on the explore button. 4. Click on any of the buttons                                                         | When the explore buttons is pressed It will show a scroll view of some buttons with some bodybuilders. When a button is pressed a popup will apear with the exercises that given bodybuidler does. When the close button is pressed the button disaprears.                                                                                                               |
| Statistics page     | Unit Test        | 1\. Run the Python file. 2. Login in. 3. Click Statistics Page                                                                                               | Once the Statistics page has been pressed. Text on the statistics of on each exercise will be displayed based of the entered info in the database for that specific user.                                                                                                                                                                                                |
| Graphing            | Integration Test | 1. Run the Python file. 2. Login in. 3. Click Statistics Page. 4. Click on given exercise. 5. Click show graph.                                             | If no exercise is selected no graph will show up however. If any of the exercises is chosen it will show the correseponding graph for the given exercise based off of the database for that user                                                                                                                                                                         |
| Final Check         | Integration Test | 1. Any of the buttons or text fields. |Checking to make sure that there are no possiblites for the program to crash based off of certain text inputs or buttons. Making sure everything works for its assigned task.|

**Table 1**: The table above shows the process that is undergone to test the final program. It contains a set of inputs with an expected outcome based. This is done to test the useability of the program and wheteher it completes what ever job it is meant to do. There are two types of test a Unit test(To test a single part of the program) and a Integration Test(To test how multiple parts of the program work together.)
...
| 19 | Adding aestetics to the page                               | To make overall app more visaully apealing                                                                                                              | Development       | 30 min             | march 5,        | C        |
...
| 22 | Statistics page visual stats                              | Providing a visul representaion of the users progression in working out based off of their inputs                                                       | Development       | 2 hours | march 6, | C |
...
https://user-images.githubusercontent.com/111752809/224201716-565df323-446b-42ad-a13b-024939fc17f9.mp4
